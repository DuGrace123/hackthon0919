...
function renderTimeline() {
  ...
  timeline.innerHTML = clips.map((clip, index) => `
    <article draggable="true" class="timeline-clip ${clip.id === state.selectedClipId ? 'selected' : ''}" data-clip-id="${clip.id}" style="--clip-width:${Math.max(90, Number(clip.duration) * state.timelineScale)}px">
      ...
      <h3 title="${escapeHtml(clip.name)}">${escapeHtml(clip.name)}</h3>
...
function renderInspector() {
  ...
  $('#clipTransition').value = clip.transition || 'none';
...
applyLanguage(false);
api('/api/project').then((data) => { hydrate(data); banner(t('connected'), 'success'); }).catch((error) => banner(error.message, 'error'));
