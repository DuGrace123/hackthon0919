...
def probe_media(ffmpeg:str, path:str, require_video:bool=True, *, timeout:float|None=None)->dict:
    name=Path(path).name
    if not Path(path).is_file():raise ValueError(f'文件不存在或不是普通文件：{name}')
    try:p=run_hidden([ffmpeg,'-hide_banner','-i',path],capture_output=True,text=True,encoding='utf-8',errors='replace',timeout=timeout)
